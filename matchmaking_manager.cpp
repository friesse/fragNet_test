...
MatchmakingManager *MatchmakingManager::GetInstance() {
  if (!s_globalInstance) {
    logger::error(
        "MatchmakingManager::GetInstance() called but no global instance set!");
  }
  return s_globalInstance;
}

void MatchmakingManager::DestroyGlobalInstance() { s_globalInstance = nullptr; }

// Constructor with dependency injection
MatchmakingManager::MatchmakingManager(std::shared_ptr<IDatabase> database,
  ...
  logger::info(
      "MatchmakingManager initialized with config: %zu players per team",
      m_config.playersPerTeam);
}

// Thread-safe queue addition with validation
bool MatchmakingManager::AddPlayerToQueue(
    uint64_t steamId, SNetSocket_t socket, const PlayerSkillRating &rating,
  ...
  try {
    ...
    entry->preferredMaps =
        preferredMaps.empty() ? m_config.mapPool : preferredMaps;
    ...
    entry->preferredMaps.erase(
        std::remove_if(entry->preferredMaps.begin(), entry->preferredMaps.end(),
                       [this](const std::string &map) {
                         return std::find(m_config.mapPool.begin(),
                                          m_config.mapPool.end(),
                                          map) == m_config.mapPool.end();
                       }),
    ...
    logger::info(
        "Player %llu added to matchmaking queue (MMR: %u, Bracket: %u)",
...
void MatchmakingManager::ProcessMatchmakingQueue() {
  ...
  logger::info("Match %llu created with %zu players on %s:%u", match->matchId,
               candidates.value().size(), server->address.c_str(),
               server->port);
...
std::optional<std::vector<std::shared_ptr<QueueEntry>>>
MatchmakingManager::FindMatchCandidates() {
  ...
  std::sort(allPlayers.begin(), allPlayers.end(),
            [](const std::shared_ptr<QueueEntry> &a,
               const std::shared_ptr<QueueEntry> &b) {
  ...
  for (size_t i = 0; i <= allPlayers.size() - matchSize; ++i) {
    std::vector<std::shared_ptr<QueueEntry>> candidates(
        allPlayers.begin() + i, allPlayers.begin() + i + matchSize);
...
std::optional<PlayerSkillRating>
MatchmakingManager::GetPlayerRating(uint64_t steamId) const {
  if (!m_database) {
  ...
  try {
    ...
    logger::error("Failed to get player rating for %llu: %s", steamId,
                  e.what());
    return std::nullopt;
  }
}

bool MatchmakingManager::UpdatePlayerRating(
    uint64_t steamId, const PlayerSkillRating &newRating) {
  if (!m_database) {
    logger::error("Database interface not available");
    return false;
  }
  ...
  try {
    ...
    logger::error("Failed to update player rating for %llu: %s", steamId,
                  e.what());
    return false;
  }
}
...
std::optional<std::shared_ptr<Match>>
MatchmakingManager::GetMatchByPlayer(uint64_t steamId) const {
...
bool Match::AllPlayersAccepted() const {
  for (const auto &player : teamA) {
    if (player && !player->acceptedMatch.load())
      return false;
  }
...
void MatchmakingManager::UpdateMatchState(uint64_t matchId,
                                          MatchState newState) {
  std::unique_lock<std::shared_mutex> lock(m_matchMutex);
  ...
  if (matchIt != m_activeMatches.end()) {
    ...
    logger::info("Match %llu state updated to %d", matchId,
                 static_cast<int>(newState));
  }
}

std::optional<std::shared_ptr<Match>>
MatchmakingManager::GetMatch(uint64_t matchId) const {
  std::shared_lock<std::shared_mutex> lock(m_matchMutex);
...
MatchmakingManager::QueueStatistics
MatchmakingManager::GetQueueStatistics() const {
...
std::shared_ptr<Match> MatchmakingManager::CreateMatch(
  ...
  match->readyUpDeadline =
      std::chrono::steady_clock::now() + m_config.readyUpTime;
...
void MatchmakingManager::DistributePlayersToTeams(
    std::shared_ptr<Match> match,
    const std::vector<std::shared_ptr<QueueEntry>> &players) {
  if (players.size() != m_config.playersPerTeam * 2) {
    logger::error("Invalid player count for team distribution: %zu",
                  players.size());
    return;
  }
...
std::optional<std::string> MatchmakingManager::SelectMapForMatch(
    const std::vector<std::shared_ptr<QueueEntry>> &players) const {
...
void MatchmakingManager::CancelMatchInternal(uint64_t matchId,
  ...
  for (const auto &player : playersToRequeue) {
    AddPlayerToQueue(player->steamId, player->socket, player->skillRating,
                     player->preferredMaps);
  }

  logger::info("Match %llu cancelled: %s", matchId, reason.c_str());
}

void MatchmakingManager::BuildMatchmakingHello(
    CMsgGCCStrike15_v2_MatchmakingGC2ClientHello &message, uint64_t steamId) {
  ...
  if (matchIt != m_playerToMatch.end()) {
    ...
    if (match != m_activeMatches.end() &&
        match->second->state.load() == MatchState::IN_PROGRESS) {
...
void MatchmakingManager::BuildMatchReservation(
    CMsgGCCStrike15_v2_MatchmakingGC2ClientReserve &message, const Match &match,
    uint64_t steamId) {
  message.set_serverid(match.matchId);
  // Note: direct_udp_ip requires inet_addr from <arpa/inet.h> (Linux) or
  // <winsock2.h> (Windows)
  ...
  // Inject Tournament Draft data if enabled
  if (TunablesManager::GetInstance().IsTournamentDraftEnabled()) {
    auto tourneyState = WebAPIClient::GetInstance().GetTournamentState();
    if (tourneyState.active) {
      auto *res = message.mutable_reservation();
      auto *preMatch = res->mutable_pre_match_data();
      auto *draft = preMatch->mutable_draft();

      draft->set_event_id(20); // Example Event ID
      draft->set_event_stage_id(tourneyState.phase);
      draft->set_team_id_0(tourneyState.teamA);
      draft->set_team_id_1(tourneyState.teamB);
      draft->set_maps_count(7); // BO3 usually involves 7 map pool
      draft->set_maps_current(0);

      // Populate drafts if available in state
      // for (int mapId : tourneyState.drafts) { ... }
      // For now just basic setup to trigger UI
    }
  }

  // Note: following fields not available in current protobuf schema
  // MatchState state = match.state.load();
  // message.set_reservation_stage(state == MatchState::WAITING_FOR_CONFIRMATION
  // ? 1 : 2); message.add_map(match.mapName); auto* token =
  // message.mutable_encrypted_steamid(); token->assign(match.matchToken);
}

void MatchmakingManager::BuildMatchUpdate(
  ...
  // match.createdTime); message.set_est_wait_time(elapsed.count());
}
...
bool MatchmakingManager::ArePlayersCompatible(const QueueEntry &p1,
                                              const QueueEntry &p2) const {
