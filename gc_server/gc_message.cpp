...
std::string_view GCMessageRead::ReadString() {
  ...
  for (uint32_t i = m_offset; i < m_size; i++) {
    if (m_data[i] == '\0') {
      std::string_view result{reinterpret_cast<const char *>(&m_data[m_offset]),
                              i - m_offset};
...
GCMessageWrite::GCMessageWrite(uint32_t type,
  ...
  [[maybe_unused]] bool result =
      message.SerializeToArray(m_buffer.data() + protobufOffset, protobufSize);
  assert(result);
}

GCMessageWrite::GCMessageWrite(uint32_t type) : m_type{type} {
  // write the non protobuf messge hader
  // mikkotoodo using GameStructMsgHeader is wrong here!!! we should be using
  // the fat one however we're not sending these to the game (yet) so it doesn't
  // matter
  WriteUint32(m_type);
  WriteUint32(0);
  WriteUint64(0);
...
void GCMessageWrite::WriteData(const void *data, uint32_t size) {
  ...
  memcpy(m_buffer.data() + old_size, bytes, size);
}
